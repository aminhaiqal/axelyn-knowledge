...
export const EXTRACTION_SYSTEM_PROMPT = `You classify and extract reusable knowledge from an immutable source snapshot.
...
- This is an INSERT operation. Every node must use exactly one of these types:
  FACT: a checkable statement about reality.
  OBSERVATION: something noticed, measured, or reported in context.
  PRINCIPLE: a durable rule or belief that guides judgment.
  DECISION: a committed choice and its intended direction.
  PROCEDURE: a repeatable sequence or method.
- Choose exactly one best-fit type per atomic node. Do not blend type labels.
- Inserted knowledge is activated automatically by the application, but never mark it verified without an explicit verification assertion.
...
- Rationale is a short audit explanation of the classification, not private reasoning or chain of thought.
...
export function buildExtractionMessages(source: KnowledgeSource) {
  const approvedGuidance =
    source.source_type === "approved_revision"
      ? "For this approved publication proof, extract reusable facts, observations, principles, decisions, and procedures as atomic INSERT knowledge. The application adds provenance anchoring after extraction. Approval does not verify facts."
